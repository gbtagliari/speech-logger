...
struct RecordingGuardTests {
    ...
    func discardsTooShortLoud() {
        ...
        #expect(guardCheck.evaluate(duration: 0.2, windowEnergies: speech(loud: 10, in: 10)) == .discardTooShort)
    }

    @Test("duration is checked before energy, so the verdict is unambiguous")
    func durationWinsOverSilence() {
        #expect(guardCheck.evaluate(duration: 0.1, windowEnergies: silence(windows: 5)) == .discardTooShort)
    }

    @Test("the minimum-duration boundary is inclusive of acceptance")
    func durationBoundaryInclusive() {
        let loud = speech(loud: 40, in: 50)
        #expect(guardCheck.evaluate(duration: 0.999, windowEnergies: loud) == .discardTooShort)
        #expect(guardCheck.evaluate(duration: 1.0, windowEnergies: loud) == .accept)
    }
    ...
    func discardsDigitalSilence() {
        ...
        #expect(guardCheck.evaluate(duration: 300, windowEnergies: silence(windows: 15000)) == .discardSilent)
    }
    ...
    func discardsSingleSpike() {
        ...
        sequence[42] = 0.35
        #expect(guardCheck.evaluate(duration: 2.0, windowEnergies: sequence) == .discardSilent)
    }

    @Test("sustained speech is accepted")
    func acceptsSustainedSpeech() {
        #expect(guardCheck.evaluate(duration: 8.0, windowEnergies: speech(loud: 300, in: 400)) == .accept)
    }
    ...
    func acceptsSparseSpeechInLongRecording() {
        ...
        #expect(guardCheck.evaluate(duration: 300, windowEnergies: speech(loud: 3000, in: 15000)) == .accept)
    }
    ...
    func accepts350msDictation() {
        ...
        let dictationGuard = RecordingGuard(minimumDuration: 0.35)
        #expect(dictationGuard.evaluate(duration: 0.35, windowEnergies: speech(loud: 12, in: 17)) == .accept)
    }
    ...
    func verdictIsDurationInvariant() {
        let short = guardCheck.evaluate(duration: 2.0, windowEnergies: speech(loud: 20, in: 100))
        let long = guardCheck.evaluate(duration: 20.0, windowEnergies: speech(loud: 200, in: 1000))
        #expect(short == .accept)
        #expect(short == long)
    }
    ...
    func loudWindowFloorInjectable() {
        ...
        #expect(strict.evaluate(duration: 5, windowEnergies: sequence) == .discardSilent)
        #expect(lenient.evaluate(duration: 5, windowEnergies: sequence) == .accept)
    }
    ...
    func replaysSilentDoubleTap() {
        #expect(guardCheck.evaluate(duration: 3.34, windowEnergies: RecordedEnergy.silentDoubleTap) == .discardSilent)
    }

    @Test("real speech at normal speaking distance is accepted")
    func replaysNormalSpeech() {
        #expect(guardCheck.evaluate(duration: 6.40, windowEnergies: RecordedEnergy.normalSpeech) == .accept)
    }

    @Test("a real 280 ms utterance is accepted on the energy test")
    func replaysShortUtterance() {
        let dictationGuard = RecordingGuard(minimumDuration: 0.35)
        #expect(dictationGuard.evaluate(duration: 1.04, windowEnergies: RecordedEnergy.shortUtterance) == .accept)
    }
    ...
    func recordedSamplesLeaveMargin() {
        ...
        #expect(strict.evaluate(duration: 6.40, windowEnergies: RecordedEnergy.normalSpeech) == .accept)
        let lenient = RecordingGuard(loudWindowFloor: 0.004, minimumLoudFraction: 0.01)
        #expect(lenient.evaluate(duration: 3.34, windowEnergies: RecordedEnergy.silentDoubleTap) == .discardSilent)
    }
    ...
    func spikeDiscardedAtTheShortestRecording() {
        ...
        sequence[25] = 0.4
        #expect(guardCheck.evaluate(duration: 1.0, windowEnergies: sequence) == .discardSilent)
    }
    ...
    func acceptsEmptyWindowSequence() {
        ...
        #expect(guardCheck.evaluate(duration: 5.0, windowEnergies: []) == .accept)
    }
}
